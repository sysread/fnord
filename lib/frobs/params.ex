defmodule Frobs.Params do
  @moduledoc """
  Helpers for normalizing frob specs and validating/coercing parameter values.

  This module converts spec maps into a predictable shape, provides a
  deterministic parameter ordering, and offers coercion/validation helpers
  used by the interactive prompt layer.

  Supports JSON Schema composition keywords (`anyOf`, `oneOf`, `allOf`) in
  addition to flat `type`-based schemas. `$ref`/`$defs` are not supported.
  """
  ...
  defp normalize_schema_value(key, value) when key in @composition_keywords and is_list(value) do
    Enum.map(value, fn
      sub when is_map(sub) -> normalize_schema(sub)
      other -> other
    end)
  end

  defp normalize_schema_value("properties", value) when is_map(value) do
    value
    |> Enum.map(fn {k, v} -> {to_string(k), normalize_schema(v)} end)
    |> Map.new()
  end

  defp normalize_schema_value("items", value) when is_map(value) do
  ...
  def validate_and_coerce_param(schema, value) when is_map(schema) do
    case resolve_schema_type(schema) do
      {:ok, type} ->
        coerce_by_type(type, schema, value)

      {:composition, keyword, sub_schemas} when keyword in ["anyOf", "oneOf"] ->
  ...
  defp coerce_any_of(sub_schemas, value) when is_list(sub_schemas) do
    result =
  ...
  defp coerce_all_of(sub_schemas, value) when is_list(sub_schemas) do
  ...
  defp merge_properties(acc, schema) do
    case Map.get(schema, "properties") do
      ...
      new_props when is_map(new_props) ->
        existing = Map.get(acc, "properties", %{})
        Map.put(acc, "properties", Map.merge(existing, new_props))
    end
  end
  ...
  defp merge_enum(acc, schema) do
    case Map.get(schema, "enum") do
      ...
      new_enum when is_list(new_enum) ->
  ...
  defp coerce_string(nil, _enum), do: {:ok, nil}
  ...
  defp maybe_check_enum({:ok, v}, enum) when is_list(enum) do
  ...
  @spec resolve_schema_type(map()) ::
          ...
          | {:error, :unresolvable}
  def resolve_schema_type(schema) when is_map(schema) do
