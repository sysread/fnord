defmodule Frobs.Prompt do
  ...
  coerced values. It relies on `Frobs.Params` for validation/coercion.
  ...
  def prompt_for_params(spec, ui \\ UI) do
    with {:ok, normalized} <- Frobs.Params.normalize_spec(spec),
         {:ok, params} <- Frobs.Params.param_list(normalized) do
  ...
  defp non_interactive_collect(params, normalized) do
    ...
    if missing != [] do
      ...
      Frobs.Params.validate_all_args(normalized, defaults)
    end
  end
  ...
  defp interactive_collect(params, normalized, ui) do
    ...
    case result do
      {:ok, answers} ->
        case Frobs.Params.validate_all_args(normalized, answers) do
  ...
  defp prompt_property(name, schema, _required?, ui) do
    ...
    case Frobs.Params.resolve_schema_type(schema) do
      {:ok, type} ->
  ...
  defp prompt_by_type(_type, _name, label, %{"enum" => choices} = _schema, ui)
  ...
  defp prompt_composition(keyword, sub_schemas, name, label, schema, ui) do
    case Frobs.Params.nullable_schema?(keyword, sub_schemas) do
      ...
      :not_nullable when keyword == "allOf" ->
        merged = Frobs.Params.merge_schemas(sub_schemas)
  ...
  defp prompt_nullable(name, label, non_null_schema, ui) do
    case Frobs.Params.resolve_schema_type(non_null_schema) do
      {:ok, type} ->
        ...
        if raw in [nil, ""] do
          ...
          case Frobs.Params.validate_and_coerce_param(non_null_schema, raw) do
            {:ok, coerced} -> {:ok, coerced}
  ...
  defp prompt_type_chooser(sub_schemas, label, ui) do
    ...
    case Frobs.Params.validate_and_coerce_param(chosen_schema, val) do
      {:ok, coerced} -> {:ok, coerced}
  ...
  defp parse_and_validate_json(raw, schema, label, ui) do
    case Jason.decode(raw || "") do
      {:ok, parsed} ->
        case Frobs.Params.validate_and_coerce_param(schema, parsed) do
          {:ok, coerced} ->
            {:ok, coerced}
  ...
  defp collect_array_items(_name, items_schema, ui, acc) do
    ...
    cond do
      ...
      true ->
        case Frobs.Params.validate_and_coerce_param(items_schema, raw) do
          {:ok, coerced} ->
