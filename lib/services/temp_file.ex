defmodule Services.TempFile do
  @moduledoc """
  Tree-scoped service for creating temporary files via Briefly.
  ...
  def start_link(_opts) do
    with {:ok, pid} <- GenServer.start_link(__MODULE__, :ok) do
  ...
  def mktemp(opts \\ []) do
    GenServer.call(Services.Instance.fetch!(__MODULE__), {:mktemp, opts})
  end
