defmodule Settings.Skills do
  ...
  ## Effective enablement (project override)

  If a project is selected and that project's `skills` key is present in settings,
  the project list is authoritative (even if empty).

  Otherwise, the global `skills` list is used.

  This differs intentionally from `Settings.Frobs`, which uses a union of global
  and project lists.
  ...
  - If a project is selected and that project has a `skills` key in settings,
    the effective list is exactly that project list.
  - Otherwise, effective list is the global list.
  ...
  def effective_enabled() do
    case Settings.get_selected_project() do
      {:ok, pn} ->
        if project_has_skills_key?(pn) do
          MapSet.new(list({:project, pn}))
        else
          MapSet.new(list(:global))
        end
  ...
  defp project_has_skills_key?(pn) when is_binary(pn) do
    settings = Settings.new()
    project = Settings.get_project_data(settings, pn) || %{}
    Map.has_key?(project, "skills")
  end
