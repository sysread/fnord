defmodule AI.Message.Assistant do
  @moduledoc """
  Assistant reply message. Wire shape:
  ...
  @doc """
  Build an assistant message from a binary or pre-built content-parts list. A
  plain binary is wrapped as a single `output_text` part.
  """
  @spec new(binary() | [content_part()]) :: t()
  def new(text) when is_binary(text) do
    %__MODULE__{content: [%{type: "output_text", text: text}]}
  end
  ...
  @impl AI.Message
  def for_transcript(%__MODULE__{} = msg) do
    case text(msg) do
      "" -> nil
      txt -> "# ASSISTANT:\n#{txt}"
    end
  ...
  legacy chat-completions shape (`%{role: "assistant", content: "binary"}`)
  so v0 conversation files migrate cleanly.
  """
  @spec from_map(map()) :: t()
  def from_map(raw) do
    ...
    parts =
      case content do
        ...
        text when is_binary(text) -> [%{type: "output_text", text: text}]
        nil -> []
      end
  ...
  defp normalize_part(%{} = part) do
    type = AI.Message.get(part, :type) || "output_text"
