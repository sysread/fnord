defmodule AI.Endpoint.Venice do
  ...
  - 429 -> `{:retry, :throttled, nil}` (Venice does not document a
    Retry-After header, so we let the harness's exponential backoff
    handle the wait).
  ...
  def endpoint_error_classify(status, body, _headers, transport_reason) do
    case {status, body, transport_reason} do
      ...
      # Throttling. Venice does not document Retry-After; rely on the
