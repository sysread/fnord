defmodule AI.Responses.Compaction do
  @moduledoc """
  Compaction utilities for AI.Responses.

  Mirrors AI.Completion.Compaction behavior to preserve parity.
  """
  ...
  defp split_preserve_last_k_rounds(msgs, k) when is_integer(k) and k >= 0 do
    ...
    if length(comp_indices) <= k do
      ...
      last_k = comp_indices |> Enum.reverse() |> Enum.take(k) |> Enum.reverse()
      keep_start = last_k |> Enum.map(&round_start_for_completion(msgs, &1)) |> Enum.min()
  ...
  def partial_compact(state, opts) do
    ...
    if older == [] do
      ...
      |> case do
        {:ok, [summary_msg]} ->
          assembled = []
          assembled = assembled |> Kernel.++(if name_msg, do: [name_msg], else: [])
          assembled = assembled |> Kernel.++([summary_msg])
          assembled = assembled |> Kernel.++(recent)
