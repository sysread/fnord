defmodule AI.Tools.RW.EditFileTest do
  use Fnord.TestCase
  ...
  setup do
    AI.Tools.RW.EditFile.override_confirm_changes(true)
    :ok
  end
  ...
  test "error on path traversal outside project" do
    ...
    assert {:error, :enoent} = AI.Tools.RW.EditFile.call(args)
  end
  ...
  test "symlink inside project is allowed", %{project: project} do
    ...
    assert {:ok, %{"diff" => diff}} = AI.Tools.RW.EditFile.call(args)
    assert diff =~ "-Z"
    assert diff =~ "+Q"
  ...
  test "case‐insensitive replacement via flags 'i'", %{project: project} do
    ...
    assert {:ok, _} = AI.Tools.RW.EditFile.call(args)
  ...
  test "empty edits yields no diff and no modification", %{project: project} do
    ...
    assert {:ok, %{"diff" => "No changes"}} = AI.Tools.RW.EditFile.call(args)
  ...
  test "error when target file does not exist" do
    ...
    assert {:error, _msg} = AI.Tools.RW.EditFile.call(args)
  end
end
