defmodule Store.EntryTest do
  ...
  describe "new_from_file_path/2" do
    test "basics", ctx do
      entry = Store.Entry.new_from_file_path(ctx.project, "a.txt")
  ...
  describe "source_file_exists?/1" do
    test "false when source file does not exist", ctx do
      ...
      # Relative file path
      refute ctx.project
             |> Store.Entry.new_from_file_path(file)
             |> Store.Entry.source_file_exists?()

      # Absolute file path
      refute ctx.project
             |> Store.Entry.new_from_file_path(path)
             |> Store.Entry.source_file_exists?()
    end
  ...
  describe "exists_in_store?/1" do
    test "false before indexing", ctx do
      file = "a.txt"
      path = mock_source_file(ctx.project, file, @text)
      entry = Store.Entry.new_from_file_path(ctx.project, path)

      refute Store.Entry.exists_in_store?(entry)
    end
  ...
  describe "is_git_ignored?/1" do
    test "false when not a git repo", ctx do
      ...
      refute Store.Entry.is_git_ignored?(entry)
    end
    ...
    test "false when not in .gitignore" do
      ...
      entry = Store.Entry.new_from_file_path(project, path)
    ...
    test "true when in .gitignore" do
      ...
      assert Store.Entry.is_git_ignored?(entry)
    end
  end
  ...
  describe "create/1 and delete/1" do
    test "basics", ctx do
      ...
      Store.Entry.create(entry)
      assert Store.Entry.exists_in_store?(entry)

      Store.Entry.delete(entry)
      refute Store.Entry.exists_in_store?(entry)
    end
  end
  ...
  describe "is_incomplete/1" do
    test "positive path", ctx do
      ...
      Store.Metadata.write(entry.metadata, %{})
      Store.Summary.write(entry.summary, "summary text")
      Store.Outline.write(entry.outline, "- outline\n  - sub-outline")
      Store.Embeddings.write(entry.embeddings, [[1, 2, 3]])

      refute Store.Entry.is_incomplete?(entry)
    end
    ...
    test "true when missing metadata", ctx do
      ...
      assert Store.Entry.is_incomplete?(entry)
    end
  ...
  describe "hash_is_current?/1" do
    test "true when file has not changed", ctx do
      ...
      assert Store.Entry.hash_is_current?(entry)
    end
    ...
    test "false when file has changed", ctx do
      ...
      refute Store.Entry.hash_is_current?(entry)
    end
  end
  ...
  describe "is_stale?/1" do
    test "positive path", ctx do
      ...
      refute Store.Entry.is_stale?(entry)
    end
    ...
    test "true when not exists_in_store?", ctx do
      ...
      assert Store.Entry.is_stale?(entry)
    end
  ...
  describe "read_source_file?/1" do
    test "basics", ctx do
      path = mock_source_file(ctx.project, "a.txt", @text)
      entry = Store.Entry.new_from_file_path(ctx.project, path)
      assert {:ok, @text} = Store.Entry.read_source_file(entry)
    end
  end
