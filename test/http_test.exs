defmodule HttpTest do
  ...
  test "retries once on 500 then succeeds" do
    # The stub runs in the calling process (this test), so a pdict counter is
    # enough to vary responses across attempts.
  ...
  test "json encode error does not retry and returns invalid_json_response" do
    # Payload with a PID cannot be encoded as JSON, so the transport is never
    # reached - no Http.Client stub needed.
