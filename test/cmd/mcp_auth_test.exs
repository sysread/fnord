defmodule Cmd.McpAuthTest do
  ...
  describe "mcp auth happy path" do
    setup do
    ...
    test "auth succeeds and prints summary" do
      {stdout, stderr} =
        capture_all(fn -> Cmd.Mcp.run(%{timeout: 5000}, [:mcp, :auth], ["authsrv"]) end)
  ...
  describe "mcp auth missing server" do
    test "prints not found error" do
      log = capture_log(fn -> Cmd.Mcp.run(%{}, [:mcp, :auth], ["nope"]) end)
  ...
  describe "provider rejects dynamic redirect" do
    ...
    test "shows exact redirect_uri to register" do
      log = capture_log(fn -> Cmd.Mcp.run(%{}, [:mcp, :auth], ["rejectsrv"]) end)
  ...
  describe "timeout path" do
    ...
    test "prints timeout error" do
      log = capture_log(fn -> Cmd.Mcp.run(%{timeout: 1}, [:mcp, :auth], ["timeoutsrv"]) end)
