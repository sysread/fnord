...
`Util.Clipboard` (`lib/util/clipboard.ex`) is the seam over the system clipboard: the `Clipboard` hex package shells out to pbcopy/xclip, so a copy overwrites whatever the user has on their clipboard. `impl/0` resolves the `:clipboard` Globals key (narrow-seam pattern, like `:http_client`), defaulting to the passthrough in `Util.Clipboard.Default`. Unlike the other narrow seams, `Fnord.TestCase` defaults the key to an inert success stub (`StubClipboard`) rather than a loud unstubbed mock - clipboard copy is an incidental side effect of the ask flow, and the right default for tests is "succeeds without touching the developer's clipboard".

`Util.Exec` (`lib/util/exec.ex`) is the subprocess sibling: outward-facing shell-outs that have no dedicated facade route through it (`:exec` Globals key, mirroring `System.cmd/3` and `System.find_executable/1`). Current callers: `Notifier` (osascript/notify-send/dunstify - desktop notifications are an external mutation, same class as the clipboard), `Cmd.Upgrade` (the escript install), `Validation.Rules` (configured validation commands), and the review decomposer's `gh` calls. Git stays on its own `GitCli` facade; `Util.Exec` is for everything else. Tests get the loud unstubbed mock - same posture as `:http_client` - because spawning a subprocess from a test is never incidental: an unscripted call dies with a Mox error instead of popping a real notification. Raw `System.cmd` for outward-facing effects outside `lib/git_cli/` is a seam hole, same rule as git.
