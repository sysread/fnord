...
| Endpoint | `AI.Endpoint.OpenAI`, `AI.Endpoint.Venice`, `AI.Endpoint.DeepSeek` | URL + provider-specific error classification. Venice's classifier maps 402 to `{:fail, :payment_required, ...}`; OpenAI's includes Cloudflare-plaintext detection. |
...
| Request builder | `AI.Provider.RequestBuilder.OpenAI`, `AI.Provider.RequestBuilder.Venice`, `AI.Provider.RequestBuilder.DeepSeek` | API key acquisition (env var lookup), header assembly (Authorization scheme), and JSON payload assembly. Honors `AI.Model` capability flags - drops fields the model does not accept; raises on `web_search?` against a non-search model. Venice builder encodes web search via `venice_parameters` and drops verbosity. |
| Response parser | `AI.Provider.ResponseParser.OpenAI`, `AI.Provider.ResponseParser.Venice`, `AI.Provider.ResponseParser.DeepSeek` | Turn raw HTTP success/error bodies into the `{:ok, :msg, ...}` / `{:ok, :tool, ...}` / `{:error, ...}` tuples the orchestration loop in `AI.Completion` understands. OpenAI parser owns the `:context_length_exceeded` extraction; Venice parser appends a "Sources:" section to the assistant text when `venice_parameters.web_search_citations` is present, and surfaces 402 with a clear payment-required message. |
| Web search | `AI.Provider.WebSearch.OpenAI`, `AI.Provider.WebSearch.Venice`, `AI.Provider.WebSearch.DeepSeek` | Provider-native web search. OpenAI runs a sub-completion against a search-preview model; Venice runs a single inline completion with `web_search?: true`. Single string-in, string-out contract for `AI.Tools.WebSearch`. |
| Model catalog | `AI.Model.OpenAI`, `AI.Model.Venice`, `AI.Model.DeepSeek` | Named profile factories (`smart/0`, `balanced/0`, `web_search/0`, ...). Each profile is an `AI.Model.t` populated with model identifier, context window, default reasoning level, and capability flags. Venice model picks come from `scratch/venice-models.md`. |
